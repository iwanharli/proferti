  ...
  <div class="relative min-h-screen bg-white font-['Plus_Jakarta_Sans',sans-serif] overflow-x-hidden">
    ...
    <header class="relative pt-32 pb-44 overflow-hidden bg-slate-900">
      ...
      <div class="container mx-auto px-4 relative z-10">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          <div class="relative lg:h-[600px] hidden lg:block animate-slide-up">
             <div class="absolute inset-0 bg-gradient-to-tr from-accent/20 to-transparent rounded-[60px] blur-3xl opacity-50"></div>
             
             <div class="relative h-full w-full rounded-[60px] overflow-hidden border border-white/10 shadow-2xl group">
                    ...
                    <div class="bg-white/10 backdrop-blur-2xl border border-white/20 p-8 rounded-[40px] shadow-2xl">
                       <div class="flex items-center justify-between">
                          <div>
                             ...
                             <h4 class="text-3xl font-black text-white tracking-tight mb-2">{{ slide.title }}</h4>
                             <div class="flex items-center gap-2 text-slate-300 font-bold text-sm">
                                <LucideMapPin :size="14" class="text-accent" /> {{ slide.location }}
                             </div>
                          </div>
                          <div class="bg-accent text-white px-6 py-4 rounded-2xl font-black text-sm shadow-glow">
                             {{ slide.price }}
                          </div>
                       </div>
                    </div>
                ...
                <div class="space-y-2">
                   ...
                   <div class="relative group">
                      <select v-model="quickFilter.city" class="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 font-black text-primary text-lg outline-none cursor-pointer appearance-none group-focus-within:border-accent transition-all">
                        ...
                        <option v-for="c in ['Jakarta', 'Bandung', 'Surabaya', 'Tangerang', 'Bali']" :key="c" :value="c">{{ c }}</option>
                      </select>
    ...
    <section class="py-44 relative overflow-hidden">
       <div class="absolute inset-0 bg-slate-900 pointer-events-none">
          ...
          <div class="absolute inset-0 bg-gradient-to-b from-white via-transparent to-white opacity-100"></div>
       </div>

       <div class="container mx-auto px-4 relative z-10 text-center">
          <div class="max-w-4xl mx-auto bg-white/80 backdrop-blur-3xl p-16 rounded-[60px] border border-white shadow-[0_50px_100px_-20px_rgba(0,0,0,0.1)7]">
    ...
    <section class="py-32">
       <div class="container mx-auto px-4">
          <div class="flex flex-col md:flex-row items-center justify-between gap-10 mb-20 border-b border-slate-100 pb-12">
             ...
             <NuxtLink to="/developers" class="text-slate-400 hover:text-accent font-black text-xs uppercase tracking-widest transition-colors flex items-center gap-2">Lihat Semua Partner <LucideArrowRight :size="16" /></NuxtLink>
          </div>
          <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
             <NuxtLink v-for="d in developers.slice(0, 5)" :key="d.id" :to="`/developers/${d.id}`" 
             ...
             >
                <img v-if="d.logo && !brokenLogos.has(d.id)" 
                  :src="d.logo" 
...
import { 
  LucideHome, LucideSearch, LucideMapPin, LucideArrowRight, 
...
// Fetch Developers
const { data: devPayload } = await useFetch<{ developers: any[] }>('/api/developers')
...
function goSearch() {
  ...
  if (quickFilter.price) {
    ...
    if (min) query.minPrice = (parseInt(min) * 1000000).toString()
    if (max) query.maxPrice = (parseInt(max) * 1000000).toString()
  }
